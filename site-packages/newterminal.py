...
def runcommand(bashcommand, hold=True):
    """
    This function runs a command in the default terminal
    emulator in a new window
    """
    script_text = SCRIPT.format(bashcommand)

    if hold:
        hold_text = HOLDER.format(
            termcolor.colored(misc.MODULE_NAME, attrs=['underline']))
        script_text = f'{script_text}{hold_text}'
        #print(script_text)
    try:
