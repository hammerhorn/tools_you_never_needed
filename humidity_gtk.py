...
class HumidityWindow(Gtk.Window):
    def __init__(self):
        ...
        self.temp_f_box = LabelledSpinButtonBox(
            lo_hi_step=(-135.8, 131.1, .1),
            default=70,
            label_markup='<span size="small"><b>Temperature(°F):</b></span>')
        self.temp_f_box.spinbutton.connect("value-changed", self.on_click)

        #Humidity box
        self.humidity_box = LabelledSpinButtonBox(
            lo_hi_step=(0.1, 99.9, .1),
            default=45,
            label_markup='<span size="small"><b>Relative humidity(%):</b></span>')
    ...
    def on_click(self, _):
        ...
        '''
        heading_list = ["<span size='large'>absolute humidity"\
                        "</span> (<i>std. pressure</i>) =",
                        "<span size='large'>dew point</span> (<i>"\
        ...
        if approx:
            heading_list.append(
                f"<span size='large'>approximate heat index"\
                "</span>(<i>±1.3</i>) =")
            output_list.append(f"<b><span color='red'>{approx:.2f}°F</span></b>")
        heading_list.append(
            '<span size="large">vapor pressure deficit</span> '\
        ...
        output_str = '\n'.join(output_list)
        self.output.set_markup(f"<span font='monospace' size='large'><b>{output_str}</b></span>\n")
